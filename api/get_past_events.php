...
if ($row = $user->num_rows > 0) {
    ...
    if (sizeof($rows) > 0) {
        $result = array('status' => 'success', 'message' => "success getting list of past event", 'data' => $rows);
        echo json_encode($result);
    } else {
        endProcessWithMessage("No past events as of now.");
    }
} else {

    endProcessWithMessage("No past events for now.");
}
