...
WHERE members.organization_id = ? AND users.registration_status = 'pending'");
...
if ($row = $users->num_rows > 0) {
    ...
    $result = array('status' => 'success', 'message' => "success getting list of pending users", 'data' => $rows);
    echo json_encode($result);
} else {
    endProcessWithMessage("No pending users found");
}
